...
$order_price_total = $_GET['order_equipment_total'] ?? '';
...
$sql = "SELECT * FROM order_equipment a 
        ...
        inner join member m on a.member_id = m.member_id WHERE a.order_equipment_type = 'ซื้อ' AND (a.order_equipment_approve IS NULL OR a.order_equipment_approve = '') ";



// กรองตามวันที่
if ($start_date && $end_date) {
    $sql .= " AND order_equipment_date BETWEEN '$start_date' AND '$end_date'";
}

// กรองตามช่วงราคา
if ($min_price && $max_price) {
    $sql .= " AND order_equipment_total BETWEEN $min_price AND $max_price";
}
...
if ($order_price_total) {
    $order_by[] = "order_equipment_total $order_price_total";
}
if (!empty($order_by)) {
...
<body>
    <header class="header">
        ...
        <nav class="nav" style="margin-left: 20%;">
            <a href="approve_page.php" class="nav-item active">อนุมัติคำสั่งซื้อ/เช่า</a>
            ...
            <div class="sidebar-content">
                ...
                <form id="filterForm" method="GET" action="" onsubmit="return false;">
                    ...
                    <select name="order_quantity" id="filter-quantity-list" class="filter-select">
                        ...
                        <option value="ASC" <?php echo (isset($_GET['order_equipment_quantity']) && $_GET['order_equipment_quantity'] == "ASC") ? "selected" : ""; ?>>น้อยสุด-มากสุด</option>
                        <option value="DESC" <?php echo (isset($_GET['order_equipment_quantity']) && $_GET['order_equipment_quantity'] == "DESC") ? "selected" : ""; ?>>มากสุด-น้อยสุด</option> -->
                    ...
                    <select name="order_price" id="filter-price-list" class="filter-select">
                        ...
                        <option value="ASC" <?php echo (isset($_GET['order_equipment_total']) && $_GET['order_equipment_total'] == "ASC") ? "selected" : ""; ?>>น้อยสุด-มากสุด</option>
                        <option value="DESC" <?php echo (isset($_GET['order_equipment_total']) && $_GET['order_equipment_total'] == "DESC") ? "selected" : ""; ?>>มากสุด-น้อยสุด</option>
                    </select>
    ...
    <select class="select" name="option" style="margin-left: 2%;" onchange="location = this.value;">
        <option value="approve_page.php" selected>อนุมัติคำสั่งซื้อ</option>
        <option value="approve_hire_page.php">อนุมัติการเช่า</option>
    </select>
